...
export const getForumDiscussion = async (req, res) => {
  try {
    const forumDiscussions = await ForumDiscussion.findAll({
      include: [
        {
          model: Users,
    ...
    const formatComments = (comments) => {
      return comments.map((comment) => ({
        id: comment.id,
        namekomen: comment.User ? comment.User.name : null,
        photoProfileUrlkomen: comment.User ? comment.User.profile_picture_url : null,
        comment: comment.comment
      }));
    };

    const response = forumDiscussions.map((forumDiscussion) => ({
      id: forumDiscussion.id,
      name: forumDiscussion.User ? forumDiscussion.User.name : null,
      photoProfileUrl: forumDiscussion.User ? forumDiscussion.User.profile_picture_url : null,
      title: forumDiscussion.title,
      description: forumDiscussion.description,
      keyword: forumDiscussion.keyword,
      photoDiscussionUrl: forumDiscussion.photoDiscussionUrl || null,
      comments: forumDiscussion.Comments ? formatComments(forumDiscussion.Comments) : []
    }));
